...
- **Refactor: convert the Node test suite to TypeScript** — rename every `tests/**/*.mjs` unit/integration/helper file to `.ts`, use `.js` specifiers for local imports, and put the RPC harness, fixtures, and assertions under `tsc --noEmit` with `noImplicitAny` enabled, so helpers and assertions earn real parameter/return contracts. The RPC harness parses a finite typed envelope instead of exposing an open `any` index signature, and command-specific `send` payloads cross explicit parsers such as the shared `CompactionResult` contract. Replace `tests/usage-test.sh` with a typed `tests/usage-test.ts` diagnostic that runs subprocesses, fetches the OAuth usage endpoint, parses JSON, and computes metrics directly instead of shelling out to Python/`eval`; it conforms untrusted subprocess JSON, validates the turns argument, spawns Pi in its own process group so a hung run's Claude Code child cannot leak, preserves per-turn `usage-test-direct-turnN.json`/`.err` evidence, and fails the run on a failed direct turn instead of the old `|| true` zero-usage row. Update the `test:unit`/`test`/`test:usage` scripts and README globs; ShellCheck now covers only the remaining `.sh` process drivers, dropping the usage-metric `SC2153` suppression.
...
- **Refactor: keep provider queries alive across turns** — move every provider request to Agent SDK streaming input mode, push follow-up turns and steering into one persistent Claude Code process, use live model switching when only the model changes, and close or rebuild the process when session history or query options drift. Replacement queries now wait for the prior transcript writer to exit before resuming or rebuilding its session; abort prefers the SDK's graceful interrupt before falling back to a hard close. Query shutdown now declares `drain` or `force` explicitly: idle persistent and completed reentrant queries reach natural EOF and its final transcript flush, while active teardown and broken queries remain forceful. A force immediately closes its transcript writer, fences late callbacks behind the closing context, and marks the shared session for rebuild; full session clear closes reentrant children before clearing the store.
...
- **Bump: current runtime and development dependencies** — update Claude Agent SDK to 0.3.218 (Claude Code 2.1.218), Anthropic SDK to 0.114.0, Pi development packages to 0.81.1, TypeBox to 1.3.7, tsx to 4.23.1, TypeScript to 7.0.2, and Node types to 26.1.1. Declare the Agent SDK's MCP SDK and Zod runtime peers directly; store the SDK's `Query` type without a bridge-owned compatibility interface.
