...
import type { RetryableStatus } from "./core-response.js";
...
export function assistantApiFailure(error: SDKAssistantMessageError | undefined): string | null {
  switch (assistantRetryableStatus(error)) {
    case 429:
...
export function assistantRetryableStatus(
  error: SDKAssistantMessageError | undefined,
): RetryableStatus | null {
  if (error === "rate_limit") return 429;
  if (error === "overloaded") return 529;
  return null;
}

export function apiStatusFailure(status: number | null | undefined): string | null {
  const retryableStatus = apiRetryableStatus(status);
  if (retryableStatus === 429) return "Claude API rate limit (HTTP 429)";
  if (retryableStatus === 529) return "Claude API overloaded (HTTP 529)";
  return null;
}

export function apiRetryableStatus(status: number | null | undefined): RetryableStatus | null {
  return status === 429 || status === 529 ? status : null;
}
