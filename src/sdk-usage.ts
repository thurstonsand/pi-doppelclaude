...
export function diffSdkModelUsage(current: SdkModelUsage, previous: SdkModelUsage): SdkModelUsage {
	return Object.fromEntries(Object.entries(current).flatMap(([key, usage]) => {
		...
		const delta: ModelUsage = {
			...
			cacheReadInputTokens: nonnegativeDelta(usage.cacheReadInputTokens, prior?.cacheReadInputTokens),
			cacheCreationInputTokens: nonnegativeDelta(usage.cacheCreationInputTokens, prior?.cacheCreationInputTokens),
		...
		const hasUsage = delta.inputTokens > 0 || delta.outputTokens > 0 || delta.cacheReadInputTokens > 0 ||
			delta.cacheCreationInputTokens > 0 || delta.webSearchRequests > 0 || delta.costUSD > 0;
...
export function accountSdkModelUsage(
	...
	aggregate.totalTokens = aggregate.input + aggregate.output + aggregate.cacheRead + aggregate.cacheWrite;
...
export function reconcileSdkModelUsage(
	...
	const servedModel = accounting.servedModels.length === 1 ? canonicalModels.get(accounting.servedModels[0]) : undefined;
	if (servedModel) {
