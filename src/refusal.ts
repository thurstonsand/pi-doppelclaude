import type { SDKModelRefusalFallbackMessage, SDKModelRefusalNoFallbackMessage } from "@anthropic-ai/claude-agent-sdk";
...
export function refusalEntryData(
	...
	return {
		...
		servedModel: message.subtype === "model_refusal_fallback" ? canonicalClaudeModelId(message.fallback_model) : null,
...
export function renderRefusalEntry(
	...
	if (!options.expanded) {
		...
		line(key
			? theme.fg("warning", `⚠ ${headline(data)} (`) + theme.fg("dim", key) + theme.fg("warning", " to expand)")
