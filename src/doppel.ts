...
function importPiMessages(
	...
	debug(`importPiMessages: ${messages.length} pi msgs → ${anthropicMessages.length} anthropic msgs`);
	debug(
	...
	if (repaired.length !== anthropicMessages.length) {
		debug(`importPiMessages: repairToolPairing ${anthropicMessages.length} → ${repaired.length} msgs`);
	}
