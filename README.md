...
Behind the scenes, pi's tools are bridged to Claude Code but it should all work like normal in pi. Bash commands get a 120-second default timeout (matching Claude Code's default) since pi's bash has no timeout by default. Skills in pi are copied over to Claude Code's system prompt in the default `append` prompt mode so should work as they would with any other pi provider.
...
- `systemPromptMode` — `"append"` keeps the Claude Code preset and appends pi's AGENTS.md/skills; `"replace"` sends a pi-derived custom system prompt instead (default `"append"`)
- `appendSystemPrompt` — append pi's AGENTS.md and skills in `"append"` mode (default `true`)
- `settingSources` — CC filesystem settings to load; defaults to `[]` in `"replace"` mode and to `["user", "project"]` when `appendSystemPrompt: false` in `"append"` mode
