...
try {
  const pkg = JSON.parse(
    readFileSync(join(DIR, "node_modules/@anthropic-ai/claude-agent-sdk/package.json"), "utf8"),
  );
  ...
  try {
    for await (const msg of query({
      ...
      if (msg.type === "assistant" && msg.error)
        assistantError = { error: msg.error, text: msg.message?.content?.[0]?.text ?? null };
      if (msg.type === "result") {
        result = msg;
        break;
      }
...
function mdTable(rows) {
  const head =
    "| model | variant | requested id | status | served model | context | max out | error |";
  const sep = "|---|---|---|---|---|---|---|---|";
  const body = rows.map((r) => {
    const [m, v] = r.requestedId.includes("[1m]")
      ? [r.requestedId.replace("[1m]", ""), "[1m]"]
      : [r.requestedId, "bare"];
    const served =
      (r.served ?? [])
        .map((s) => `${s.servedModel}@${s.contextWindow}/${s.maxOutputTokens}`)
        .join(" ") || "—";
    const errParts = [];
...
async function run(plan) {
  ...
  for (const requestedId of combos) {
    ...
    console.log(
      `${r.status} served=${served}${errParts.length ? ` err=${errParts.join(" · ")}` : ""}`,
    );
  }
  const stamp = iso();
  const report = {
...
function compare() {
  ...
  if (!pro || !max) {
    console.log(`need both a pro-* and max-* JSON (found pro=${!!pro} max=${!!max})`);
    process.exit(1);
  }
