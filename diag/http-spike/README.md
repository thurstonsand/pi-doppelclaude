...
Remaining verification limits: the browser UI fork action could not be exercised because the orb browser lacked an authenticated Amp UI session. New-key history import and branch isolation passed the direct SDK diagnostic, but that does not prove UI fork marker behavior. In-flight crash, automatic restart policy, and an isolated multi-minute heartbeat/cancellation measurement remain untested. Temporary test-agent tooling was removed after verification.
