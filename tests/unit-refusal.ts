...
const FALLBACK: SDKModelRefusalFallbackMessage = {
	...
	content: "Fable 5's safeguards flagged this message. Switched to Opus 4.8. Send feedback with /feedback or learn more: https://support.claude.com/en/articles/15363606",
...
function entryFor(data: RefusalEntryData): CustomEntry<RefusalEntryData> {
	return { type: "custom", id: "e1", timestamp: "2026-07-27T00:00:00.000Z", customType: REFUSAL_CUSTOM_TYPE, data } as CustomEntry<RefusalEntryData>;
}

// The host hands the renderer its theme, so the test hands it one that paints nothing.
const plainTheme = { fg: (_color: string, text: string) => text, bg: (_color: string, text: string) => text } as unknown as Theme;
...
describe("Claude refusal entries", () => {
	...
	it("collapses to a headline carrying whatever key the user bound to expand", () => {
		setKeybindings(new KeybindingsManager({ "app.tools.expand": { defaultKeys: "ctrl+o", description: "Toggle tool output" } } as never, { "app.tools.expand": "alt+e" } as never));
	...
	it("records the refusal as an entry and stands in for the served-model warning", async () => {
		...
		const sdkQuery = (async function* () {
			...
			yield { type: "stream_event", event: { type: "message_start", message: { model: "claude-opus-4-8-20260401", usage: {} } } };
			yield { type: "stream_event", event: { type: "message_stop" } };
		})() as unknown as Query;

		await runtime.test.consumeQuery(sdkQuery, new Map(), fakeModel, queryCtx, { onResult() {}, onSessionId() {} });

		assert.deepEqual(entries.map((entry) => entry.customType), [REFUSAL_CUSTOM_TYPE]);
