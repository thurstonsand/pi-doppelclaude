...
import type { McpServerConfig, McpSetServersResult, Query, SDKResultMessage } from "@anthropic-ai/claude-agent-sdk";
import {
	apiStatusFailure,
	assistantApiFailure,
...
function queryWithSetMcpServers(results: McpSetServersResult[], calls: Array<Record<string, McpServerConfig>>): Query {
...
describe("Agent SDK runtime signals", () => {
	it("reconciles MCP server addition", async () => {
		...
		const sdkQuery = queryWithSetMcpServers([{ added: ["custom-tools"], removed: [], errors: {} }], calls);
	...
	it("reconciles MCP server removal", async () => {
		const calls: Array<Record<string, McpServerConfig>> = [];
		const sdkQuery = queryWithSetMcpServers([{ added: [], removed: ["custom-tools"], errors: {} }], calls);
		await reconcileMcpServers(sdkQuery, "custom-tools", true, {});
		assert.deepEqual(calls, [{}]);
	});

	it("rejects MCP reconciliation errors and missing receipts", async () => {
		const failed = queryWithSetMcpServers([{ added: [], removed: [], errors: { "custom-tools": "connection failed" } }], []);
		await assert.rejects(() => reconcileMcpServers(failed, "custom-tools", false, { "custom-tools": server }), /connection failed/);

		const unconfirmed = queryWithSetMcpServers([{ added: [], removed: [], errors: {} }], []);
		await assert.rejects(() => reconcileMcpServers(unconfirmed, "custom-tools", false, { "custom-tools": server }), /did not confirm/);
	});
	...
	it("waits for query initialization without an extra MCP status request", async () => {
		...
		sdkQuery.mcpServerStatus = async () => { throw new Error("unexpected status request"); };
	...
	it("classifies reusable, interrupted, and terminal results once", () => {
		const result = (terminal_reason: SDKResultMessage["terminal_reason"], is_error = false) => ({
			type: "result", subtype: "success", terminal_reason, is_error,
		}) as SDKResultMessage;
		...
		assert.deepEqual(classifyResult(result("aborted_streaming"), null, null), {
			type: "interrupted", message: "Claude query ended with aborted_streaming",
		});
		assert.deepEqual(classifyResult(result("blocking_limit"), "quota rejected", "quota rejected"), {
			type: "terminal", message: "quota rejected",
		});
	});
});
