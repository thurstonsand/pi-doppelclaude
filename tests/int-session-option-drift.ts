...
try {
  ...
  const response = await harness.promptAndWait(
    "What exact word did I ask you to remember? Reply only with it.",
  );
  ...
  assert.ok(
    sessionIds.every((id) => id === sessionId),
    `session ID changed across option drift: ${sessionIds.join(", ")}`,
  );
  const efforts = [...log.matchAll(/fresh streaming query.*effort=(\S+)/g)].map(
    (match) => match[1],
  );
  assert.deepEqual(
    efforts,
    ["low", "high"],
    `test precondition failed: expected low → high effort drift, saw ${efforts.join(" → ")}`,
  );
  assert.equal(
    (log.match(/fresh streaming query/g) ?? []).length,
    2,
    "thinking drift should respawn exactly once",
  );
  ...
  assert.ok(
    closeIdx >= 0 &&
      naturalEofIdx > closeIdx &&
      finalAppendIdx > naturalEofIdx &&
      stoppedIdx > finalAppendIdx &&
      writerClosedIdx >= stoppedIdx &&
      secondSpawnIdx > stoppedIdx,
    "replacement query did not await the old process's final mirror flush",
  );

  const prefix = sessionId.slice(0, 8);
  assert.ok(
    log.includes(`session-store: load writer=provider session=${prefix}`),
    "replacement query did not resume through SessionStore",
  );
  assert.ok(
    log.includes(`session-store: append writer=provider session=${prefix}`),
    "replacement query did not mirror records into SessionStore",
  );
  assert.equal(
    existsSync(getSessionPath(sessionId, cwd)),
    false,
    "first-spawn fragment survived replacement writer shutdown",
  );
  ...
  if (sessionId)
    assert.equal(
      existsSync(getSessionPath(sessionId, cwd)),
      false,
      "first-spawn fragment survived session shutdown",
    );
