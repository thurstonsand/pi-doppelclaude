...
import type { AssistantMessageEvent, Context, Message as PiMessage, Model, Tool } from "@earendil-works/pi-ai";
...
function toolUseEvents(id: string, name: string, args: string): SDKMessage[] {
	return [
		...
		{ type: "stream_event", event: { type: "content_block_start", index: 0, content_block: { type: "tool_use", id, name, input: {} } } },
...
function ccRejection(id: string): SDKMessage {
	return {
		type: "user",
		message: { role: "user", content: [{ type: "tool_result", tool_use_id: id, content: "No such tool available", is_error: true }] },
	} as unknown as SDKMessage;
}

function resultMessage(): SDKMessage {
	return { type: "result", subtype: "success", result: "", is_error: false, modelUsage: {} } as unknown as SDKMessage;
}

function toolCalls(events: AssistantMessageEvent[]) {
	return events.filter((event) => event.type === "toolcall_end").map((event: any) => ({ id: event.toolCall.id, name: event.toolCall.name }));
}
...
const rejectedTurn = [
	...prompt,
	{ role: "assistant", content: [{ type: "toolCall", id: "call_bad", name: "cc_no_such_tool__bash", arguments: { command: "ls" } }] },
	{ role: "toolResult", toolCallId: "call_bad", content: "Tool cc_no_such_tool__bash not found", isError: true },
];
...
describe("Claude Code-rejected tool calls", () => {
	...
	it("buffers the correction that arrives before pi re-enters (the incident ordering)", async () => {
		...
		const dispatched = handler({ command: "ls" }, { _meta: { "claudecode/toolUseId": "call_good" } });
		await tick();
		assert.equal(ctx.pendingToolCalls.has("call_good"), true);
		record(stream(runtime, [
			...rejectedTurn,
			{ role: "assistant", content: [{ type: "toolCall", id: "call_good", name: "bash", arguments: { command: "ls" } }] },
	...
	it("resolves the valid call and drops the rejected one when a message mixes both", async () => {
		...
		queue.push({ type: "stream_event", event: { type: "content_block_start", index: 0, content_block: { type: "tool_use", id: "call_good", name: "mcp__custom-tools__bash", input: {} } } } as unknown as SDKMessage);
		queue.push({ type: "stream_event", event: { type: "content_block_stop", index: 0 } } as unknown as SDKMessage);
		queue.push({ type: "stream_event", event: { type: "content_block_start", index: 1, content_block: { type: "tool_use", id: "call_bad", name: "bash", input: {} } } } as unknown as SDKMessage);
		queue.push({ type: "stream_event", event: { type: "content_block_stop", index: 1 } } as unknown as SDKMessage);
		queue.push({ type: "stream_event", event: { type: "message_delta", delta: { stop_reason: "tool_use" } } } as unknown as SDKMessage);
		...
		const second = record(stream(runtime, [
			...prompt,
			{ role: "assistant", content: [
				{ type: "toolCall", id: "call_good", name: "bash", arguments: { command: "ls" } },
				{ type: "toolCall", id: "call_bad", name: "cc_no_such_tool__bash", arguments: { command: "ls" } },
			] },
			{ role: "toolResult", toolCallId: "call_good", content: "file.txt" },
			{ role: "toolResult", toolCallId: "call_bad", content: "Tool cc_no_such_tool__bash not found", isError: true },
		]));
	...
	it("terminates the turn when a handler is still waiting after full delivery", async () => {
		...
		queue.push({ type: "stream_event", event: { type: "content_block_start", index: 0, content_block: { type: "tool_use", id: "call_good", name: "mcp__custom-tools__bash", input: {} } } } as unknown as SDKMessage);
		queue.push({ type: "stream_event", event: { type: "content_block_stop", index: 0 } } as unknown as SDKMessage);
		queue.push({ type: "stream_event", event: { type: "content_block_start", index: 1, content_block: { type: "tool_use", id: "call_other", name: "mcp__custom-tools__bash", input: {} } } } as unknown as SDKMessage);
		queue.push({ type: "stream_event", event: { type: "content_block_stop", index: 1 } } as unknown as SDKMessage);
		queue.push({ type: "stream_event", event: { type: "message_delta", delta: { stop_reason: "tool_use" } } } as unknown as SDKMessage);
		queue.push({ type: "stream_event", event: { type: "message_stop" } } as unknown as SDKMessage);
		await tick();
