...
const { startAndWait, stop, send, waitForEvent, waitForMatch, collectText, DEBUG_LOG, RPC_LOG } =
  harness;
...
async function waitForReentrantCountAbove(count: number, label: string) {
  ...
  throw new Error(
    `${label}: timed out waiting for a reentrant nested-runtime query (count stayed ${count})`,
  );
}

async function runNestedPrompt({
  background,
  expectedMarker,
}: {
  background: boolean;
  ...
  assert.match(
    ...
    `parent did not report nested completion. Text: ${text.slice(0, 500)}`,
  );
  await waitForReentrantCountAbove(
    beforeReentrant,
    background ? "background NestedAgent" : "foreground NestedAgent",
  );
}
...
try {
  await runNestedPrompt({
    background: false,
    expectedMarker: "PARENT-SAW-FOREGROUND-NESTED-AGENT",
  });
  ...
  assert.match(
    log,
    /mcp handler: NestedAgent \[toolu_/,
    "debug log never showed the parent NestedAgent MCP handler",
  );
  assert.doesNotMatch(
    log,
    STUCK_MARKER,
    "debug log contains a stuck-handler or stream-overwrite signature",
  );
