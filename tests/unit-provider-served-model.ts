...
const servedUsage = {
	"claude-opus-4-8": {
		inputTokens: 5, outputTokens: 7, cacheReadInputTokens: 0, cacheCreationInputTokens: 0,
		webSearchRequests: 0, costUSD: 0.01, contextWindow: 200_000, maxOutputTokens: 64_000,
		canonicalModel: "claude-opus-4-8", provider: "firstParty",
	},
};

const DETECTED = "Claude served claude-opus-4-8 instead of requested claude-opus-5; usage priced from served model";
...
function startedRuntime(warnings: string[], entries: unknown[] = []) {
	...
	const runtime = createBridgeRuntime({
		...
		modelCatalog: {
			getModels: () => [],
			async refresh() {},
			async noteServedModel(id) { observed.push(id); },
		},
	});
	runtime.setHost({
...
describe("provider served-model reporting", () => {
	it("reports a demotion when it is detected and again when the turn ends", async () => {
		...
		const result = { type: "result", subtype: "success", is_error: false, terminal_reason: "completed", result: "ok", modelUsage: servedUsage };
		await runtime.test.consumeQuery(demotedTurn(result, () => { warningsWhileStreaming = [...warnings]; }), new Map(), fakeModel, queryCtx, {
			onResult() {},
			onSessionId() {},
		});
		...
		queryCtx.beginCommand(fakeModel);
		queryCtx.currentPiStream = createAssistantMessageEventStream();
		await runtime.test.consumeQuery(demotedTurn(result, () => {}), new Map(), fakeModel, queryCtx, {
			onResult() {},
			onSessionId() {},
		});
		assert.deepEqual(warnings, [DETECTED, RECAP]);
		...
		queryCtx.currentPiStream = createAssistantMessageEventStream();
		await runtime.test.consumeQuery(demotedTurn(result, () => {}), new Map(), otherModel, queryCtx, {
			onResult() {},
			onSessionId() {},
		});
	...
	it("teaches the catalog the served model", async () => {
		...
		await runtime.test.consumeQuery(demotedTurn(undefined, () => {}), new Map(), fakeModel, queryCtx, {
			onResult() {},
			onSessionId() {},
		});
