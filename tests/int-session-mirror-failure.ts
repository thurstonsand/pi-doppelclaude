...
const runtime = createBridgeRuntime({ providerSettings: { systemPromptMode: "claude-code" }, sessionStore });
...
try {
	...
	const secondUser = { role: "user", content: "Reply only MIRROR_RECOVERED.", timestamp: Date.now() + 1 } as const;
	const secondContext: Context = { systemPrompt: "You are concise.", messages: [firstUser, failed, secondUser] };
