...
const harness = createRpcHarness({
  ...
  args: [
    ...
    "-e",
    `${process.cwd()}/tests/fixtures/tool-set-extension.ts`,
  ],
...
try {
  ...
  assert.equal(
    (log.match(/provider: fresh streaming query/g) ?? []).length,
    1,
    "tool changes rotated the Claude process",
  );
  assert.equal(
    (log.match(/provider: reconciling MCP tools without process replacement/g) ?? []).length,
    2,
    "tool changes were not reconciled twice",
  );
  assert.match(
    log,
    /provider: reconciling MCP tools without process replacement \(replace\/remove\)/,
  );
