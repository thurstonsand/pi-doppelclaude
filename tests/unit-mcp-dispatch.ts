...
async function connect(queryCtx: QueryContext) {
	...
	const config = servers[MCP_SERVER_NAME] as { instance: { connect(transport: unknown): Promise<void> } };
...
describe("MCP tool dispatch", () => {
	...
	it("dispatches arguments pi would reject instead of validating them away", async () => {
		...
		pending.resolve({ content: [{ type: "text", text: "Invalid arguments: path must be a string" }], isError: true });
		const result = await call;
		assert.deepEqual(result.content, [{ type: "text", text: "Invalid arguments: path must be a string" }]);
		await client.close();
	});
