...
function parseAssistantMessageEvent(value: unknown): AssistantMessageEvent | undefined {
	...
	if (typeof event.type !== "string") throw new Error("assistantMessageEvent.type must be a string");
...
export function parseCompactionResult(value: unknown): CompactionResult {
	...
	if (typeof result.summary !== "string") throw new Error("compaction result.summary must be a string");
...
export function parseCompactionEndEvent(message: RpcMessage): CompactionEndEvent {
	if (message.type !== "compaction_end") throw new Error(`expected compaction_end, got ${message.type}`);
...
export function createRpcHarness(opts: RpcHarnessOptions) {
		...
		pi.stdout.on("data", (chunk) => {
			...
			while (true) {
				...
				try {
					...
					rpcLog.write(`! invalid RPC message: ${error instanceof Error ? error.message : String(error)}\n`);
				}
			}
		});
	}
	...
	function send<T>(cmd: Record<string, unknown>, timeout = defaultTimeout, parse?: Parser<T>): Promise<T | void> {
		const id = `req_${++reqId}`;
		const full = { ...cmd, id };
	...
	function waitForMatch(predicate: (msg: RpcMessage) => boolean, description: string, timeout = defaultTimeout): Promise<RpcMessage> {
		return new Promise<RpcMessage>((resolve, reject) => {
			let remove = () => {};
			const timer = setTimeout(() => {
				remove();
