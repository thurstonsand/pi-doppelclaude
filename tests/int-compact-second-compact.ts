...
try {
  ...
  if (!first?.summary?.trim())
    throw new Error(`first compact returned empty summary: ${JSON.stringify(first)}`);
  ...
  if (!second?.summary?.trim())
    throw new Error(`second compact returned empty summary: ${JSON.stringify(second)}`);
