...
describe("RPC harness boundaries", () => {
  ...
  it("rejects malformed message fields", () => {
    assert.throws(
      () => parseRpcMessage({ type: "response", success: "yes" }),
  ...
  it("conforms compaction command results", () => {
    ...
    assert.throws(
      () => parseCompactionResult({ summary: "summary", tokensBefore: "42" }),
      /tokensBefore must be a finite number/,
    );
  });

  it("conforms compaction events after event matching", () => {
    const event = parseCompactionEndEvent(
      parseRpcMessage({
        type: "compaction_end",
        aborted: false,
    ...
    assert.throws(
      () =>
        parseCompactionEndEvent(
          parseRpcMessage({ type: "compaction_end", result: { summary: "summary" } }),
        ),
