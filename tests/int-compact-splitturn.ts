...
try {
  ...
  if (!compactResult.usage || compactResult.usage.totalTokens <= 0) {
    throw new Error(
      `split-turn compact returned no combined summary usage: ${JSON.stringify(compactResult)}`,
    );
  }
  ...
  if (!/after-compact-ok/.test(after)) {
    throw new Error(
      `post-compact prompt did not return expected marker. Got: ${after.slice(0, 200)}`,
    );
  }
