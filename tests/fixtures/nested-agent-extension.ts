...
async function runNestedAgent(prompt: string, ctx: ExtensionContext): Promise<string> {
	...
	if (!model) throw new Error(`NestedAgent could not resolve ${ctx.model.provider}/${ctx.model.id}`);
...
export default function (pi: ExtensionAPI) {
	...
	const agentParameters = Type.Object({
		...
		background: Type.Optional(Type.Boolean({ description: "Start the nested agent in the background" })),
	});
	pi.registerTool<typeof agentParameters>({
		name: "NestedAgent",
		label: "Nested agent",
		description: "Runs a prompt in a nested Pi ModelRuntime. Use this exactly when the user asks for NestedAgent.",
