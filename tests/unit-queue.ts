...
function extractAllToolResults(messages: QueueMessage[]) {
  ...
  return results.map((r) => {
    ...
    return {
      ...r,
      content:
        Array.isArray(r.content) && r.content.length === 1 && first?.type === "text"
          ? first.text
          : r.content,
    };
  });
  ...
  return {
    ...
    waitForResult(toolCallId: string): Promise<QueueDelivery> {
      ...
      return new Promise((resolve) => {
        pendingHandlers.set(toolCallId, { resolve });
      });
    },
...
describe("results before handlers", () => {
  it("N results queued, then N handlers resolve immediately", async () => {
    ...
    for (let i = 0; i < 3; i++)
      bridge.deliverResult({ toolCallId: `t${i}`, content: [{ type: "text", text: `r${i}` }] });
...
describe("interleaved messages in tool result sequences", () => {
  ...
  it("user message at tail, no toolResult → must find 0 and not enter delivery", async () => {
    ...
    assert.equal(
      bridge.handlersWaiting,
      1,
      "handler still stuck — caller should have skipped delivery",
    );
    bridge.drain({ content: [] });
  });
    ...
    for (let trial = 0; trial < 1000; trial++) {
      ...
      for (const op of ops) {
        if (op.type === "deliver") {
          bridge.deliverResult({
            toolCallId: `t${op.idx}`,
            content: [{ type: "text", text: `r${op.idx}` }],
          });
        } else {
