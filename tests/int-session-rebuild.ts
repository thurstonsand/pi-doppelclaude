...
test("SessionStore resume reads an atomically replaced transcript under the same UUID", { timeout: 180_000 }, async () => {
	const store = new BridgeSessionStore();
	const sessionId = crypto.randomUUID();
	seed(store, sessionId, "ALPHA");

	const first = await run(store, sessionId, "What token did I ask you to remember? Reply with just the word.", "alpha");
	assert.match(first, /alpha/i);
	assert.equal(existsSync(getSessionPath(sessionId, CWD)), false, "bridge seed must not write into ~/.claude/projects");

	seed(store, sessionId, "BETA");
	const second = await run(store, sessionId, "What token did I ask you to remember? Reply with just the word.", "beta");
	assert.match(second, /beta/i);
	assert.doesNotMatch(second, /alpha/i);
	assert.equal(existsSync(getSessionPath(sessionId, CWD)), false, "SDK should materialize resumes outside ~/.claude/projects");
});

test("SDK mirrors newly persisted records back into the authoritative store", { timeout: 180_000 }, async () => {
	const store = new BridgeSessionStore();
	const sessionId = crypto.randomUUID();
	...
	assert.ok(records.length > before, `expected mirrored records beyond ${before}, got ${records.length}`);
	assert.ok(records.every((record) => record.sessionId === sessionId), "mirror introduced a foreign session ID");
	assert.ok(records.some((record) => JSON.stringify(record).includes("MIRRORED")), "mirrored transcript omitted the response");
});
