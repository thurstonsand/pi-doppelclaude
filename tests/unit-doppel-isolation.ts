...
const foreignCallers = [
  {
    label: "a keyless auxiliary call",
  ...
  {
    label: "a foreign session",
    options: { sessionId: FOREIGN_SESSION } as SimpleStreamOptions,
    messages: secondTurn,
  },
];
...
describe("turns that are not the host's", () => {
  for (const caller of foreignCallers) {
    it(`leaves the host's idle persistent query alone for ${caller.label}, and still reuses it for the next host turn`, async () => {
      ...
      assert.equal(
        harness.spawned[0].closed,
        false,
        "the foreign turn closed the host's persistent query",
      );
    ...
    it(`does not move the shared session onto ${caller.label}'s own result`, async () => {
      const harness = makeHarness([
        ...
        {
          messages: [
            { type: "system", subtype: "init", session_id: OTHER_SESSION } as unknown as SDKMessage,
            ...answer("A Conversation", OTHER_SESSION),
          ],
        },
      ]);
      await openHostConversation(harness);
  ...
  it("does not collide with a host turn that arrives alongside it", async () => {
    ...
    assert.notEqual(
      hostCtx.turnRetry,
      null,
      "the keyless turn's death spent the host turn's one retry",
    );
  });
