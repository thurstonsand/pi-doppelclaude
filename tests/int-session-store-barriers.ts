...
class BlockingMirrorStore extends BridgeSessionStore {
	...
	readonly firstAppendStarted = new Promise<void>((resolve) => { this.markFirstStarted = resolve; });
	private readonly firstAppendReleased = new Promise<void>((resolve) => { this.releaseFirst = resolve; });
...
try {
	...
	assert.equal(terminal, false, "provider emitted its terminal result before SessionStore.append completed");

	store.release();
	const message = await completion;
	assert.equal(message.stopReason, "stop");
	assert.match(message.content.map((block) => block.type === "text" ? block.text : "").join(""), /BARRIER_OK/);
	const appendsAtResult = store.appendCount;
	await runtime.closePersistent("barrier integration");
	assert.ok(store.appendCount > appendsAtResult, "natural EOF did not deliver its final mirror append");
