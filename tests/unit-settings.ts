...
function writeSettings(
  path: string,
  doppelclaude: Record<string, unknown>,
...
describe("loadBridgeSettings", () => {
  it("loads the namespace from global Pi settings without rejecting sibling settings", () =>
  ...
  it("requires custom replacements whenever the Pi prompt is included", () =>
    withSettingsDirs(({ agentDir, cwd }) => {
  ...
  it("rejects replacements that omit any required prose", () =>
    withSettingsDirs(({ agentDir, cwd }) => {
      writeSettings(join(agentDir, "settings.json"), {
        provider: {
      ...
      assert.throws(
        () => load(cwd, agentDir),
        /must have required properties identity, toolNameNote/,
      );
    }));

  it("rejects blank replacement prose", () =>
    withSettingsDirs(({ agentDir, cwd }) => {
      writeSettings(join(agentDir, "settings.json"), {
        provider: {
          systemPromptReplacements: {
  ...
  it("defaults the prompt mode to pi after validating its replacements", () =>
    withSettingsDirs(({ agentDir, cwd }) => {
      writeSettings(join(agentDir, "settings.json"), {
        provider: {
          systemPromptReplacements: {
  ...
  it("ignores unknown keys inside doppelclaude", () =>
    withSettingsDirs(({ agentDir, cwd }) => {
      writeSettings(join(agentDir, "settings.json"), {
        provider: {
          systemPromptMode: "claude-code",
      ...
      assert.deepEqual(settings.debug, {
        enabled: true,
        logPath: join(agentDir, "doppelclaude.log"),
      });
    }));

  it("applies debug environment overrides after global file settings", () =>
    withSettingsDirs(({ agentDir, cwd }) => {
      writeSettings(join(agentDir, "settings.json"), {
  ...
  it("ignores global and project doppelclaude.json files", () =>
    withSettingsDirs(({ agentDir, cwd, projectDir }) => {
      writeSettings(join(agentDir, "settings.json"), {
        provider: { systemPromptMode: "claude-code" },
      });
  ...
  it("surfaces global Pi settings parse failures instead of silently using defaults", () =>
    withSettingsDirs(({ agentDir, cwd }) => {
      writeFileSync(join(agentDir, "settings.json"), "{");
      assert.throws(() => load(cwd, agentDir), /failed to load global Pi settings.*settings\.json/);
    }));
});
