...
function assertReadFile(details: CompactionDetails | undefined, suffix: string) {
  ...
  assert(
    Array.isArray(readFiles),
    `readFiles missing/invalid in details: ${JSON.stringify(details)}`,
  );
  assert(
...
try {
  ...
  harness.addListener((msg) => {
    if (msg.type === "compaction_start" && msg.reason === "threshold") {
      ...
      if (!disableAutoPromise) {
        disableAutoPromise = send({ type: "set_auto_compaction", enabled: false }, 30_000).catch(
          (err) => {
            throw new Error(
              `failed to disable auto-compaction after threshold start: ${err.message}`,
            );
          },
        );
      }
  ...
  assert(
    thresholdEnds.length === 1,
    `expected exactly one threshold compaction_end, got ${thresholdEnds.length}`,
  );
  assert(endEvent.aborted === false, `threshold compaction aborted: ${JSON.stringify(endEvent)}`);
  assert(
    endEvent.result?.summary?.trim(),
    `threshold compaction returned empty summary: ${JSON.stringify(endEvent)}`,
  );
  assert(
  ...
  assert(
    thresholdStarts.length === 1,
    `second threshold compaction_start fired (${thresholdStarts.length} total)`,
  );
  assert(
    thresholdEnds.length === 1,
    `second threshold compaction_end fired (${thresholdEnds.length} total)`,
  );

  const debugLog = readFileSync(DEBUG_LOG, "utf8");
  assert(
    /session_before_compact: takeover/.test(debugLog),
    "debug log missing compact takeover marker",
  );
  assert(
  ...
  assert(
    compactSpawns >= 2,
    `expected at least 2 isolated compact summary spawns, got ${compactSpawns}`,
  );
  assert(
    !/currentPiStream overwritten/.test(debugLog),
    "debug log reported currentPiStream overwrite",
  );
