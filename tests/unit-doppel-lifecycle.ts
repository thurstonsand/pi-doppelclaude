...
function makeHarness(scripts: QueryScript[]) {
	...
	const runtime = createBridgeRuntime({
		...
		queryFactory: ({ prompt, options }) => {
			...
			void (async () => {
				for await (const _ of prompt) { /* the turn's prompt */ }
				queue.end();
			})();
...
function answer(text: string, sessionId?: string): SDKMessage[] {
	return [
		...
		{ type: "result", subtype: "success", result: "", is_error: false, modelUsage: {}, ...(sessionId ? { session_id: sessionId } : {}) },
...
describe("doppel lifecycle", () => {
	it("reconstitutes a guest by resuming the session its closed query left behind", async () => {
		...
		assert.equal(spawned[1].options.resume, "guest-cc-session", "the guest did not resume its own session");
	...
	it("leaves no doppel and no transcript behind for a keyless call", async () => {
		...
		assert.equal(runtime.test.getStoredSession(rebuilt), null, "the ephemeral left its transcript in the store");
	});
	...
	it("keeps a host and a guest out of each other's sessions", async () => {
		...
		assert.deepEqual(runtime.test.getHostSession(), hostSession, "the guest turn wrote the host's session");
		...
		assert.equal(spawned.length, 2, "the host turn respawned instead of pushing into its warm query");
	...
	it("demotes the outgoing host to a guest and closes its query", async () => {
		...
		assert.equal(outgoing.session?.sessionId, "host-cc-session", "the outgoing host lost its session");
	...
	it("upgrades a doppel that spoke before pi named its host", async () => {
		...
		assert.equal(spawned[1].options.resume, "host-cc-session", "the upgraded host did not resume its session");
		assert.equal(early.context.persistent, true, "the upgraded host did not get a persistent query");
		assert.deepEqual(texts(events), ["ok"]);
	});
});
