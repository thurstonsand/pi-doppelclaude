...
try {
	...
	const sessionPrefix = log.match(/syncResult: path=clean-start[\s\S]*?turn complete, session=([a-f0-9]+)/)?.[1];
	assert.ok(sessionPrefix, "no clean-start session ID prefix in bridge log");
	const projectDir = getProjectDir(cwd);
	const fragmentName = readdirSync(projectDir).find((name) => name.startsWith(sessionPrefix) && name.endsWith(".jsonl"));
	...
	assert.ok(shutdownLog.includes(`provider: deleted first-spawn session fragment ${sessionPrefix}`));
