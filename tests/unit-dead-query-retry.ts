...
function stream(
  ...
  return runtime.test.streamClaudeAgentSdk(
    ...
    { sessionId: HOST_SESSION, ...options },
  );
}
...
function answeredToolCall(priorMessages: unknown[]) {
  return [
    ...
    {
      role: "assistant",
      content: [{ type: "toolCall", id: TOOL_CALL_ID, name: "bash", arguments: { command: "ls" } }],
    },
...
describe("dead query retry", () => {
  ...
  it("tells the user how to re-authenticate when a revoked token outlives the retry", async () => {
    ...
    assert.match(
      error,
      /claude \/login/,
      "a revoked credential is only actionable in Claude Code's own terms",
    );
  });
  ...
  it("replays a turn pushed into a query that had already died", async () => {
    const { runtime, spawned } = makeHarness([
      {
        stayOpen: true,
        failControl: QUERY_CLOSED,
        messages: [...textEvents("first"), successResult],
      },
  ...
  it("replays a tool-result continuation the warm query died answering", async () => {
    ...
    assert.equal(
      spawned[1].promptsEnded,
      false,
      "the host's replay must come back warm, not as a one-shot",
    );
  ...
  it("carries steering forward when the query dies holding it", async () => {
    ...
    assert.equal(
      ...
      STEERING,
      "the steering never reached the warm query",
    );
  ...
  it("replays a reentrant query's continuation as another one-shot", async () => {
    ...
    assert.equal(
      spawned[2].promptsEnded,
      true,
      "a reentrant replay must stay a one-shot: prompted once, input closed",
    );
