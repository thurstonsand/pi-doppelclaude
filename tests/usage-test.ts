...
function parseUsage(value: unknown): UsageSnapshot {
	...
	if (raw.seven_day_opus != null) snapshot.seven_day_opus = toWindow(raw.seven_day_opus, "seven_day_opus");
	if (raw.seven_day_sonnet != null) snapshot.seven_day_sonnet = toWindow(raw.seven_day_sonnet, "seven_day_sonnet");
	return snapshot;
}
...
function printUsage(usage: UsageSnapshot): void {
	...
	if (usage.seven_day_sonnet) console.log(`  seven_day_sonnet: ${usage.seven_day_sonnet.utilization}%`);
}
...
function printMetricsTable(rows: PiUsage[]): Metrics {
	console.log(`${"Turn".padEnd(6)}  ${"Input".padStart(8)}  ${"CacheRd".padStart(8)}  ${"CacheWr".padStart(8)}  ${"Output".padStart(8)}  ${"Cost".padStart(10)}`);
	console.log("------  --------  --------  --------  --------  ----------");
	...
	for (const usage of rows) {
		...
		console.log(`${String(total.turns).padEnd(6)}  ${String(usage.input).padStart(8)}  ${String(usage.cacheRead).padStart(8)}  ${String(usage.cacheWrite).padStart(8)}  ${String(usage.output).padStart(8)}  $${cost}`);
	}

	console.log("------  --------  --------  --------  --------  ----------");
	console.log(`${"Total".padEnd(6)}  ${String(total.input).padStart(8)}  ${String(total.cacheRead).padStart(8)}  ${String(total.cacheWrite).padStart(8)}  ${String(total.output).padStart(8)}  $${total.cost}`);

	const cacheTotal = total.input + total.cacheRead + total.cacheWrite;
	if (cacheTotal > 0) console.log(`Cache hit rate: ${round((total.cacheRead * 100) / cacheTotal, 1)}%`);
	return total;
}
...
async function runPi(env: NodeJS.ProcessEnv, prompts: string[], logFile: string): Promise<void> {
	...
	const args = ["--no-session", "-ne", "-e", DIR, "--model", `anthropic/${MODEL}`, "--mode", "json", ...promptArgs];
	...
	const child = spawn("pi", args, { cwd: DIR, env, timeout: 600_000, detached: true, stdio: ["ignore", "pipe", "pipe"] });
...
function parseClaudeTurn(raw: string, jsonLog: string): ClaudeTurnResult {
	...
	if (typeof parsed !== "object" || parsed === null) throw new Error(`claude turn JSON was not an object (see ${jsonLog})`);
	const record = parsed as { session_id?: unknown; total_cost_usd?: unknown; usage?: unknown };
	if (typeof record.session_id !== "string" || !record.session_id) throw new Error(`claude turn JSON missing session_id (see ${jsonLog})`);
	if (typeof record.usage !== "object" || record.usage === null) throw new Error(`claude turn JSON missing usage (see ${jsonLog})`);
	...
	return {
		...
		usage: {
			...
			cache_read_input_tokens: requireNumber(u.cache_read_input_tokens, `turn usage.cache_read_input_tokens (${jsonLog})`),
			cache_creation_input_tokens: requireNumber(u.cache_creation_input_tokens, `turn usage.cache_creation_input_tokens (${jsonLog})`),
...
function runClaudeTurn(env: NodeJS.ProcessEnv, prompt: string, resumeId: string | null, turnIndex: number): ClaudeTurnResult {
	const jsonLog = join(LOGDIR, `usage-test-direct-turn${turnIndex}.json`);
	const errLog = `${jsonLog}.err`;
	const args = ["--model", CC_MODEL, "--output-format", "json", "-p", prompt, "--permission-mode", "bypassPermissions"];
	if (resumeId) args.push("--resume", resumeId);

	const result = spawnSync("claude", args, { cwd: DIR, env, encoding: "utf8", timeout: 120_000, maxBuffer: 32 * 1024 * 1024, stdio: ["ignore", "pipe", "pipe"] });
	writeFileSync(jsonLog, result.stdout ?? "");
	writeFileSync(errLog, result.stderr ?? "");

	if (result.error) throw new Error(`claude turn ${turnIndex} failed: ${result.error.message} (see ${errLog})`);
	if (result.signal) throw new Error(`claude turn ${turnIndex} terminated by signal ${result.signal} (see ${errLog})`);
	if (result.status !== 0) throw new Error(`claude turn ${turnIndex} exited with code ${result.status} (see ${errLog})`);
...
async function runDirect(env: NodeJS.ProcessEnv, token: string): Promise<{ metrics: Metrics; delta: number }> {
	console.log("\n==========================================");
...
function printComparison(a: { metrics: Metrics; delta: number }, b: { metrics: Metrics; delta: number }): void {
	console.log("\n==========================================");
	console.log("  Comparison");
	console.log("==========================================");

	const col = (value: string) => value.padStart(12);
	const row = (label: string, av: string, bv: string, diff: string) => `  ${label.padEnd(17)}${col(av)}  ${col(bv)}  ${col(diff)}`;
	const signed = (value: number) => (value >= 0 ? `+${value}` : `${value}`);

	console.log(row("", "Bridge", "Direct", "Diff"));
	console.log(row("", "------", "------", "----"));
	console.log(row("Input tokens", `${a.metrics.input}`, `${b.metrics.input}`, signed(a.metrics.input - b.metrics.input)));
	console.log(row("Cache read tokens", `${a.metrics.cacheRead}`, `${b.metrics.cacheRead}`, signed(a.metrics.cacheRead - b.metrics.cacheRead)));
	console.log(row("Cache write tokens", `${a.metrics.cacheWrite}`, `${b.metrics.cacheWrite}`, signed(a.metrics.cacheWrite - b.metrics.cacheWrite)));
	console.log(row("Output tokens", `${a.metrics.output}`, `${b.metrics.output}`, signed(a.metrics.output - b.metrics.output)));
	console.log(row("API-equiv cost", `$${a.metrics.cost}`, `$${b.metrics.cost}`, `$${round(a.metrics.cost - b.metrics.cost, 6)}`));
	console.log(row("Cache hit rate", `${cacheHitRate(a.metrics)}%`, `${cacheHitRate(b.metrics)}%`, `${round(cacheHitRate(a.metrics) - cacheHitRate(b.metrics), 1)}%`));
	console.log(row("Usage delta (5h)", `${a.delta}%`, `${b.delta}%`, `${round(a.delta - b.delta, 2)}%`));
