...
Agent SDK 0.3.219 bundles the corresponding Claude Code runtime and exposes several contracts that replace bridge-owned inference:
...
- `Query.setMcpServers()` updates the live process's MCP surface. The runtime diffs normalized server definitions, applies additions/removals through the control channel, verifies the response, and rebuilds only if reconciliation fails. The first prompt waits for `initializationResult()` before entering the live MCP surface. `reconnectMcpServer()` is reserved for an explicitly failed existing server, not used as a general retry loop.
...
- Rate-limit events and result errors distinguish API 429 from overload 529 and expose stable limit type, utilization, reset windows, Opus/Sonnet weekly buckets, and credit eligibility. The provider converts stable structured fields into Pi warnings and terminal errors; human message text is preserved only as detail. Experimental usage APIs remain unused until Anthropic removes their instability warning.
- Structured `terminal_reason` values identify completed, interrupted, blocked, and failed turns. The runtime also emits finer command lifecycle frames, but the public declaration gap below prevents using them for correctness.

The installed 0.3.219 declarations expose less than the release prose implies, so Phase 7 adopts only the typed public boundary. `SDKControlInitializeResponse` has no MCP status or capabilities fields, and SDK-owned in-process servers are absent from `mcpServerStatus()` even when connected; the bridge awaits `initializationResult()` before releasing the first prompt, but cannot positively verify an omitted SDK server and does not issue a status request that cannot report it. The runtime emits `command_lifecycle` messages, but 0.3.219 exports no message type for them and omits them from `SDKMessage`; consuming their payload would require the bridge-local structural protocol this phase removes. Likewise, `Query.interrupt()` accepts no `cancel_queued` option and its implementation returns only `still_queued`, despite the exported internal response type mentioning `cancelled`. The bridge therefore stamps each input UUID, consumes the typed interrupt receipt, aborted assistant flag, and terminal reason, and force-closes on any surviving or unverifiable queue rather than parsing untyped lifecycle frames. Stable rate-limit events expose Opus/Sonnet weekly buckets and credit eligibility, while arbitrary `model_scoped` windows exist only on the explicitly experimental usage method; the bridge does not call it. API retry status and assistant error categories permit stable 429/rate-limit and 529/overload classification, but error result messages do not expose a numeric HTTP status. These are SDK contract gaps, not invitations to cast the wire protocol.
...
- [x] Phase 7: Adopt Agent SDK 0.3 runtime control and integrity signals
  ...
  - Files: `src/bridge-runtime.ts`, `src/provider-stream.ts`, `src/query-state.ts`, `src/session-store.ts`, `src/sdk-signals.ts`, `src/sdk-usage.ts`, focused unit/authenticated integration tests, README, TODO, CHANGELOG, and this design.
  - Work: Kept the SDK `Query` type as the sole live-query contract; moved normal turn accounting to cumulative-delta `modelUsage`, summing SDK `costUSD`, pricing component tokens from each canonical served model, and warning on fallback while preserving Pi's requested-model identity. Removed tools from the process signature and reconciled changed in-process MCP definitions through confirmed remove/add `setMcpServers()` control calls, awaiting initialization before the first prompt so tool changes retain the warm process. UUID-stamped input, interrupt receipts, aborted assistant metadata, and public terminal reasons now gate reuse; surviving or unverifiable queued input forces revision-fenced rebuild. Structured rate-limit buckets, utilization/reset/credit fields, API retry status, and assistant error categories map to Pi warnings or terminal errors. Query options explicitly select batched mirroring and a 15-second load timeout. Results remain the turn mirror barrier, natural EOF remains the graceful replacement/final-flush barrier, forced close invalidates the writer revision, and mirror/load/malformed-resume failures invalidate partial storage before the next complete-history rebuild. The experimental usage method and session summary indexes remain unused. The declaration gaps above prevent typed command-lifecycle/cancelled-discard consumption and positive initialization status for omitted SDK-owned MCP servers; no structural compatibility protocol was added.
  - Validation: `npm run typecheck`, lint, and 145 unit tests pass, covering cumulative multi-model fallback pricing, MCP add/remove/replacement/failure, empty/nonempty interrupt receipts, 429/529 and model-scoped stable quota messages, malformed transcripts, forced-close revision fencing, and SDK load timeout. Authenticated tests prove MCP tool changes do not rotate Claude Code, queued steering cannot leak after interruption, a terminal result waits for a blocked mirror append, natural EOF emits its final mirror batch, option drift waits for the prior writer, and a three-attempt mirror failure invalidates partial state then rebuilds from complete Pi history. Runtime command cancellation/discard frames and arbitrary model-scoped quota lists have no public 0.3.219 message contracts, so the corresponding typed tests cannot exist; fallback pricing is deterministic in unit coverage because the authenticated service does not expose a safe way to force a fallback.
